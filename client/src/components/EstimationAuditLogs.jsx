...
function EstimationAuditLogs() {
  ...
  const fetchAuditLogs = useCallback(async () => {
    ...
    try {
      ...
      if (deletedByFilter) params.append('deletedBy', deletedByFilter)
  ...
  }, [currentPage, itemsPerPage, actionFilter, entityTypeFilter, startDateFilter, endDateFilter, deletedByFilter])
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px' }}>
              <div className="form-group">
                ...
                >
                  ...
                  <option value="quotation_deleted">Quotation Deleted</option>
                  <option value="revision_deleted">Revision Deleted</option>
                  <option value="project_variation_deleted">Project Variation Deleted</option>
              ...
              <div className="form-group">
                <label>Deleted By (User ID)</label>
                <input 
                  type="text" 
                  value={deletedByFilter} 
                  onChange={e => {
                    setDeletedByFilter(e.target.value)
                    setCurrentPage(1)
                  }}
                  ...
                  <tr key={log._id}>
                    <td data-label="Action">
                      <span className="status-badge" style={{ 
                        background: log.action === 'quotation_deleted' ? '#dc2626' : '#7c3aed',
                        color: 'white',
                        padding: '4px 8px',
                        borderRadius: '4px',
                        fontSize: '12px',
                    ...
                    <td data-label="Entity Type">{formatEntityType(log.entityType)}</td>
                    <td data-label="Entity Details">
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                        {log.entityData?.offerReference && (
                          <span><strong>Ref:</strong> {log.entityData.offerReference}</span>
                        )}
                        ...
                        {log.entityData?.createdBy && (
                          <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>
                            Created by: {typeof log.entityData.createdBy === 'object' ? log.entityData.createdBy.name : 'N/A'}
                          </span>
                        )}
                      </div>
                    </td>
                    <td data-label="Deleted By">
                      {log.deletedBy ? (
                        <div>
                          <div>{log.deletedBy.name || 'N/A'}</div>
                          <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>
                            {log.deletedBy.email || ''}
                          </div>
                          {log.deletedBy.roles && (
                            <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '2px' }}>
                              {log.deletedBy.roles.join(', ')}
                            </div>
                          )}
                        </div>
                      ) : 'N/A'}
...
export default EstimationAuditLogs
